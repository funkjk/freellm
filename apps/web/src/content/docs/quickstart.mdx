---
title: Quickstart
description: Deploy your FreeLLM gateway in under 2 minutes.
---

Three ways to get FreeLLM running. Pick whichever fits your workflow.

## Option A: One-click cloud deploy

The fastest path. Click a button, sign in, paste your provider API keys, done.

- [Deploy on Railway](https://railway.com/deploy/_0jAQr) (reads `railway.json` automatically)
- [Deploy to Render](https://render.com/deploy?repo=https://github.com/Devansh-365/freellm) (reads `render.yaml` automatically)

Your gateway is live in under 2 minutes. No clone, no Docker, no terminal.

### Vercel (serverless, opt-in Redis)

The gateway can also run as a Vercel Node function (`apps/gateway`). Set the Vercel project root to `apps/gateway`, then configure:

```bash
FREELLM_STATE_BACKEND=redis
UPSTASH_REDIS_REST_URL=...
UPSTASH_REDIS_REST_TOKEN=...
# plus at least one provider key
```

Without Redis, each serverless isolate has its own memory — rate limits and caps will not be shared. Long SSE streams may be cut by Vercel `maxDuration` (heartbeats stay enabled; truncated streams are not resumed).

## Option B: Docker

Pull the prebuilt multi-arch image from GHCR (no clone needed):

```bash
docker run -d -p 5180:3000 \
  -e GROQ_API_KEY=gsk_... \
  -e GEMINI_API_KEY=AI... \
  -e MISTRAL_API_KEY=... \
  -e NVIDIA_NIM_API_KEY=nvapi-... \
  --name freellm \
  ghcr.io/devansh-365/freellm:latest
```

Or clone and use docker-compose:

```bash
git clone https://github.com/Devansh-365/freellm.git
cd freellm
cp .env.example .env        # add your API keys
docker compose up
```

## Option C: Local development

```bash
git clone https://github.com/Devansh-365/freellm.git
cd freellm
pnpm install
cp .env.example .env
pnpm dev
```

The API runs on `http://localhost:5180` and the dashboard on `http://localhost:5181`.

## Get free API keys

Sign up for any combination of these (more keys = more capacity):

| Provider | Where to get a key |
|----------|--------------------|
| Groq | [console.groq.com](https://console.groq.com) |
| Gemini | [aistudio.google.com](https://aistudio.google.com) |
| Mistral | [console.mistral.ai](https://console.mistral.ai) |
| NVIDIA NIM | [build.nvidia.com](https://build.nvidia.com) |

## Make your first request

Once your gateway is running, point any OpenAI-compatible SDK at it:

```python
from openai import OpenAI

client = OpenAI(
    base_url="http://localhost:5180/v1",
    api_key="unused",
)

response = client.chat.completions.create(
    model="free-fast",
    messages=[{"role": "user", "content": "Hello!"}],
)

print(response.choices[0].message.content)
print("Provider used:", response.x_freellm_provider)
```

The `x_freellm_provider` field on the response tells you which upstream provider handled the request. Useful for debugging routing decisions.
