...
- **The origin must match exactly.** Protocol, host, and port all have to line up. `https://yoursite.com` and `https://www.yoursite.com` are different origins. `http://localhost:5180` and `http://localhost:5181` are different origins. Mint separate tokens for each.
